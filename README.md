...
- 🎛️ **策略系统** - 支持多种抽卡策略（80抽小保底策略、120抽井策略）和附加策略（A1-A5）
- 🧾 **资源来源明细** - 抽数/武库配额支持来源拆解与提示（版本福利、卡池赠送、加急寻访、情报书、角色池转化等）
- 💳 **充值估算分页** - 提供“全图鉴 0+1”场景下的补抽/充值需求分布（按“用到时补齐”计算）
...
2. **策略选择** - 基础策略（S1/S2）+ 附加策略（A1/A2/A3/A4/A5）
3. **蒙特卡洛模拟** - 1,000 ~ 20,000 次试验
4. **结果分析** - 期望值、中位数总结、分位点、概率分布、资源来源明细
5. **充值估算** - 第二分页：全图鉴 0+1 场景的充值/补抽需求估算
...
- ✅ 武库配额：角色抽卡获取、版本发放
- ✅ 可选：规划时跳过第一个版本的版本福利（用于“当前版本资源已获取/已花掉”场景）
...
2. **配置规划参数**
   ...
   - 规划版本数（1-8）
   - 每版本卡池数（1-3）
   - （可选）第一个版本不计入版本资源：用于表示“第一个版本的版本福利已获取/已花掉，只计算后续版本可获得资源”
...
3. **选择抽卡策略**
   ...
   - **附加策略**：
     ...
     - A3: 凑情报书（为下个池准备）
     - A4: 最后版本用光所有资源
       - A5: 武器井优先（影响武器池申领门槛：8次井 vs 4次保底）

4. **设置模拟参数**
   - 模拟次数：1,000 / 5,000 / 10,000 / 20,000 / 100,000
...
切换到“要充多少抽才能拿下后续全图鉴？”分页后：
- 输入同样的当前资源与规划配置
- 运行后会输出：不充值可获得资源、以及达到全图鉴 0+1 的充值需求分布（抽数/武库配额）
