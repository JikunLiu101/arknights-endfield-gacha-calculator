...
页面顶部提供两个分页切换按钮（两个分页都保持挂载，切换时保留输入状态）：
- 看看你的全图鉴概率
- 要充多少抽才能拿下后续全图鉴？（全图鉴 0+1 充值估算）
...
**应用场景**：
1. **规划版本数** (1-8个版本)
2. **模拟次数** (五档：1000/5000/10000/20000/100000)
...
interface TrialsSliderProps {
  label: "模拟次数";
  options: [1000, 5000, 10000, 20000, 100000];
  value: number;
  onChange: (value: number) => void;
}
```
...
```css
- 采用离散五档设计
- 刻度标记: "1K" "5K" "10K" "20K" "100K"
- 当前值显示: "1,000次" "5,000次" "10,000次" "20,000次" "100,000次"
- 警告提示: 10K/20K 时显示 "可能需要较长时间"，100K 时显示 "可能需要很长时间"
```
